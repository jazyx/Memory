...
export default function Memory(props) {
  ...
  const [ player, setPlayer ] = useState(json.player)
  const playerCount = Object.values(json.players).length
  ...
  const pairFound = () => {
    ...
    cards[flippedCards[0]].found =
      ...
      json.players[player].name

    json.players[player].score += 1
    setToFind(toFind - 1)
  ...
  const score = json.players.map(({name, score}, index) => {
